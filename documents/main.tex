...
\textbf{Tầng ứng dụng (Application Layer):} bao gồm các instance NestJS được tổ chức theo mô hình Backend-for-Frontend \cite{b7}. Mỗi instance phục vụ riêng một kênh người dùng thông qua tập hợp các Controller chuyên biệt, thực hiện việc tiếp nhận yêu cầu, xác thực phiên làm việc, xác thực dữ liệu đầu vào và điều phối luồng xử lý xuống các tầng bên dưới. Việc tách biệt này cho phép tối ưu hóa cấu trúc phản hồi theo nhu cầu riêng của từng nền tảng mà không làm phức tạp hóa giao diện lập trình ứng dụng.

\textbf{Tầng dịch vụ (Service Layer)}: tập trung toàn bộ quy tắc nghiệp vụ cốt lõi của hệ thống quản lý bệnh viện. Các Service tại tầng này được triển khai dưới dạng các đơn vị xử lý thuần túy, không phụ thuộc vào cơ sở dữ liệu hay giao thức truyền tải. Tại đây diễn ra các quyết định nghiệp vụ quan trọng như kiểm tra ràng buộc lịch khám, tính toán quyền truy cập hồ sơ, sinh đơn thuốc điện tử và xử lý quy trình thanh toán, đảm bảo tính nhất quán và khả năng kiểm thử độc lập.
...
\textbf{Tầng lưu trữ (Persistence Layer)}: sử dụng đồng thời ba hệ quản trị cơ sở dữ liệu phù hợp với đặc thù dữ liệu. PostgreSQL đảm bảo tính toàn vẹn và nhất quán cho các thực thể có quan hệ phức tạp; MongoDB phục vụ lưu trữ tài liệu phi cấu trúc và nhật ký hoạt động với hiệu năng ghi cao; Redis được sử dụng làm bộ nhớ đệm và cơ chế lưu trữ phiên thời gian thực, hỗ trợ các tính năng thông báo tức thì và trò chuyện trực tuyến.
...
\item \textbf{Chọn nhãn}: sử dụng bảng tần suất đã tiền xử lý (nếu có) hoặc thống kê trực tiếp từ dữ liệu để xây dựng tập nhãn mục tiêu, chỉ giữ các mã có mức xuất hiện đủ lớn nhằm bảo đảm tính ổn định khi huấn luyện.
\item \textbf{Chia dữ liệu}: áp dụng chia nhóm theo \texttt{subject\_id} để tránh rò rỉ bệnh nhân, hình thành ba tập train/validation/test với tỷ lệ cân đối cho mọi giai đoạn.
\item \textbf{Vector hoá}: áp dụng \texttt{TfidfVectorizer} với cấu hình \(n\)-gram từ đơn đến song ngữ, sử dụng sublinear TF, chuẩn hoá \(L_{2}\) và giới hạn kích thước từ vựng để kiểm soát bộ nhớ.
...
\item \textbf{Huấn luyện}: \texttt{OneVsRestClassifier} bao bọc \texttt{SGDClassifier} với hàm mất mát logistic, regularization \(L_{2}\) và cơ chế early stopping. Sau khi hội tụ, trọng số được chuyển về \texttt{float32} để tiết kiệm dung lượng.
\item \textbf{Đóng gói và suy luận}: mô hình, vectorizer và bộ mã hoá nhãn được lưu thành một gói duy nhất. Hàm tiện ích \texttt{predict\_topk} nhận danh sách ghi chú, biến đổi qua pipeline và trả về các nhãn xác suất cao nhất để phục vụ đánh giá cũng như xuất báo cáo.
\end{itemize}
...
\item \textbf{FastAPI} triển khai microservices AI, xử lý gợi ý chẩn đoán từ ghi chú y khoa và chatbot hỗ trợ bệnh nhân.
\item \textbf{Gemini 2.5 Flash} tích hợp làm lõi AI thông minh cho chatbot tư vấn 24/7 hỗ trợ bệnh nhân khi đến khám tại phòng khám.
...
\item \textbf{Supabase} cung cấp nền tảng backend-as-a-service cho lưu trữ đối tượng, xác thực và sự kiện thời gian thực, giúp tăng tốc phát triển các module phụ trợ \cite{b8}.
\end{itemize}
...
\item Resend gửi email thông báo
\item Supabase lưu trữ ảnh, tài liệu
\item AWS SNS gửi mã xác thực OTP qua SMS
...
\item Dữ liệu huấn luyện: $\sim$200 k hồ sơ MIMIC-IV.
\item Đầu ra: xác suất của từng mã ICD ($\approx$ 2k nhãn)
...
Trong đồ án này, chúng tôi đã trình bày chi tiết quy trình xây dựng và triển khai hệ thống Revita -- một nền tảng hỗ trợ chẩn đoán y khoa trên cả web và di động. Hệ thống tích hợp các công nghệ hiện đại như NextJS, NestJS, Electron, React Native, FastAPI, Gemini, kết hợp với mô hình dự đoán bệnh được huấn luyện từ tập dữ liệu y khoa nổi tiếng MIMIC-IV. Nhờ đó, hệ thống cung cấp gợi ý bệnh lý chính xác, nhanh chóng dựa trên ghi chú bệnh án, tuổi và giới tính, hỗ trợ hiệu quả cho bác sĩ trong công tác sàng lọc và chẩn đoán ban đầu.

Mặc dù đã đạt được những kết quả khả quan, hệ thống vẫn còn hạn chế lớn là chưa sử dụng dữ liệu thực tế từ chính người dùng để huấn luyện và cải thiện mô hình. Trong thời gian tới, nhóm sẽ tập trung: Thu thập và tích hợp dữ liệu nội sinh từ hệ thống Revita để tái huấn luyện mô hình định kỳ, nâng cao độ chính xác và phù hợp với thực tế lâm sàng tại Việt Nam. Cải thiện trải nghiệm người dùng: tối ưu giao diện, tăng tốc độ phản hồi, hỗ trợ đa ngôn ngữ và tích hợp voice-to-text cho ghi chú bệnh án. Nâng cấp mô hình dự đoán: thử nghiệm các mô hình ngôn ngữ y khoa chuyên biệt (như BioClinicalBERT), bổ sung xử lý phủ định và ngữ cảnh phức tạp. Mở rộng nền tảng: tích hợp với hệ thống bệnh viện (HIS), và triển khai thử nghiệm thực tế tại các cơ sở y tế.
...
\begin{thebibliography}{00}
    ...
    \bibitem{b7}
    G. Magolan, J. Bell, D. Guijarro, A. de Peretti, and P. Housley, \textit{Nest.js: A Progressive Node.js Framework}. [Online]. Available: https://leanpub.com/nestjs
    
    \bibitem{b8}
    A. Author, ``Supabase vs Firebase: Evaluation of Performance and Development of Progressive Web Apps,'' Bachelor's thesis, Dept. Inform. Technol., Metropolia Univ. Appl. Sci., Helsinki, Finland, May 2022.
